    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'yarp_control_msgs.msg.Position')
            logger.debug(
...
class Position(metaclass=Metaclass_Position):
    ...
    _fields_and_field_types = {
        ...
        'ref_velocities': 'sequence<double>',
    }
    ...
    def __init__(self, **kwargs):
        ...
        self.ref_velocities = array.array('d', kwargs.get('ref_velocities', []))
    ...
    def __eq__(self, other):
        ...
        if self.positions != other.positions:
            return False
        if self.ref_velocities != other.ref_velocities:
            return False
        return True
    ...
    @builtins.property
    def ref_velocities(self):
        """Message field 'ref_velocities'."""
        return self._ref_velocities

    @ref_velocities.setter
    def ref_velocities(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'ref_velocities' array.array() must have the type code of 'd'"
            self._ref_velocities = value
            return
        if __debug__:
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'ref_velocities' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._ref_velocities = array.array('d', value)
