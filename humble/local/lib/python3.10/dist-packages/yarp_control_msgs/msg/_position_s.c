...
bool yarp_control_msgs__msg__position__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("yarp_control_msgs.msg._position.Position", full_classname_dest, 40) == 0);
  }
  yarp_control_msgs__msg__Position * ros_message = _ros_message;
  {  // names
  ...
  {  // ref_velocities
    PyObject * field = PyObject_GetAttrString(_pymsg, "ref_velocities");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      Py_ssize_t size = view.len / sizeof(double);
      if (!rosidl_runtime_c__double__Sequence__init(&(ros_message->ref_velocities), size)) {
        PyErr_SetString(PyExc_RuntimeError, "unable to create double__Sequence ros_message");
        PyBuffer_Release(&view);
        Py_DECREF(field);
        return false;
      }
      double * dest = ros_message->ref_velocities.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'ref_velocities'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
PyObject * yarp_control_msgs__msg__position__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("yarp_control_msgs.msg._position");
  ...
  {  // ref_velocities
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "ref_velocities");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->ref_velocities.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->ref_velocities.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->ref_velocities.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
