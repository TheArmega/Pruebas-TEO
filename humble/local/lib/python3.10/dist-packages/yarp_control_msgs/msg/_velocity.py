    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'yarp_control_msgs.msg.Velocity')
            logger.debug(
...
class Velocity(metaclass=Metaclass_Velocity):
    ...
    _fields_and_field_types = {
        ...
        'ref_accelerations': 'sequence<double>',
    }
    ...
    def __init__(self, **kwargs):
        ...
        self.ref_accelerations = array.array('d', kwargs.get('ref_accelerations', []))
    ...
    def __eq__(self, other):
        ...
        if self.velocities != other.velocities:
            return False
        if self.ref_accelerations != other.ref_accelerations:
            return False
        return True
    ...
    @builtins.property
    def ref_accelerations(self):
        """Message field 'ref_accelerations'."""
        return self._ref_accelerations

    @ref_accelerations.setter
    def ref_accelerations(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'ref_accelerations' array.array() must have the type code of 'd'"
            self._ref_accelerations = value
            return
        if __debug__:
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'ref_accelerations' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._ref_accelerations = array.array('d', value)
